...
This repository now includes a runnable baseline scaffold with:

- Canonical DOM/data models in `core/`
- Baseline encoder in `dom_encoder/`
- Canonical action generation in `action_space/`
- PUCT-based MCTS implementation in `search/`
- Heuristic reward shaping in `reward/`
- Online replan-and-execute loop in `runner/`
- Simple evaluation harness in `eval/`
- Trace recorder in `traces/`
- Unit tests in `tests/`

## Near-term roadmap

1. Replace mock environment with real browser integration.
2. Expand DOM schema for richer semantic and accessibility signals.
3. Add learned policy/value model interface.
4. Add trace viewer and richer regression benchmarks.
5. Optimize for parallel simulation and lower latency.
